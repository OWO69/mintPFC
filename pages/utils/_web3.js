...
export const mintWhitelist = async (account,numberOfTokens, proof) => {
  ...
  const amount = (numberOfTokens * 0.0069).toString();
  ...
  const result = sampleNFT.methods.mintWhitelist(numberOfTokens,proof).send({ from: account, value: amountToWei }).then((result) => {
    console.log(`✅ Check out your transaction on Etherscan: https://goerli.etherscan.io/tx/` + result);
      return {
        success: true,
        status: `✅ Check out your transaction on Etherscan: https://goerli.etherscan.io/tx/` + result
        };
  }).catch((err) => {
  ...
  export const mintPublic = async (account, numberOfTokens) => {
    ...
    const amount = (numberOfTokens * 0.0077).toString();
    const amountToWei = web3.utils.toWei(amount, 'ether');
